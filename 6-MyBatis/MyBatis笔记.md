...
```
## 4. 创建mapper接口
>MyBatis中的mapper接口相当于以前的dao。但是区别在于，mapper仅仅是接口，我们不需要提供实现类。这里需要UserMapper.xml在映射文件中给出实现的sql语句。
```java
...
<configuration>
    ...
    <mappers>
		...
		在resources下创建包时，文件之间需要用\分割，不能用.。如果用点，则是文件名直接带点。
        注意：
...
```
- <img src="C:\Users\86158\AppData\Roaming\Typora\typora-user-images\image-20240326202309822.png" alt="image-20240326202309822" style="zoom:50%;" /> 



## 2. jdbc.properties

要使用jdbc.properties，需要在mybatis-config.xml中指明
...
- fetchType：当开启了全局的延迟加载之后，可以通过该属性手动控制延迟加载的效果，单独设置某个查询为立即加载或者延迟加载，fetchType="lazy(延迟加载)|eager(立即加载)"
...
- `choose、when、otherwise`相当于`if...else if..else`，相当于`switch`
...
- 属性：  
	...
	- item：表示集合或数组中的每一个数据  
	- separator：设置循环体之间的分隔符，分隔符前后默认有一个空格，如` , `，也可以设置`or`
...
- 批量添加
  - 这里参数是List类型的map，因此在指定属性名时，需要加上`emp.`
