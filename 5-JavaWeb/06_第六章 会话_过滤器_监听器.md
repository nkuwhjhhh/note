...
+ 请求转发时,请求域可以传递数据`请求域内一般放本次请求业务有关的数据,如:查询到的所有的部门信息`，重定向不可以用请求域传递数据
...
+ 过滤器配置代码（==filter-mapping的顺序决定了过滤器的工作顺序==）（以注解形式写的过滤器，其工作顺序是类名排序）
...
+ 源码
  + 注意三个方法
    ...
    + String[] servletNames() default {};：以别名方式指定要过滤的servlet
    + String[] urlPatterns() default {};：以路径方式指定要过滤的servlet
