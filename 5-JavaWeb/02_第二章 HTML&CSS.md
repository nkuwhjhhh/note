...
+ 6 HTML中不严格区分字符串使用单双引号，嵌套时单双引号交替使用
...
+ href属性用于定义连接
    + href中可以使用绝对路径,以/开头，始终以一个固定路径作为基准路径作为出发点。该固定路径是以127.0.0.0加端口号为准。
...
+ input标签,主要的表单项标签,可以用于定义表单项
    ...
    + type, input标签的属性之一,用于定义表单项类型
        + text   文本框
        + password 密码框
        + submit 提交按钮：可以根据文本框中的文本和密码以？的形式跳转
        + reset    重置按钮：清空文本框和密码框的内容
