...
<script type="module" setup>
    ...
    // 将一个reactive响应式对象中的多个属性转换成多个ref响应式对象，这里的counter和data.counter是同一个
...
+ 总的来说，`v-if` 有更高的切换开销，而 `v-show` 有更高的初始渲染开销。因此，如果需要**频繁切换**，则使用 `v-show` 较好；如果在运行时绑定条件**很少改变**，则 `v-if` 会更合适。
