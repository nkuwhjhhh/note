...
public class UserServlet  extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        resp.setHeader("Content-Type", "text/html");//可以将响应报文的类型设置为"text/html"
    }
}
```
...
``` java
@WebServlet(
    // name是别名，和servlet中的name一样
    // value是映射路径，就是在浏览器中输入网址时的后半部分，用于指明资源的部分
    // urlPatterns同上
    // initParams: 配置初始化参数
        name = "userServlet",
        //value = "/user",
...
6. 不建议在service方法中修改成员变量，在并发请求时，会引发线程安全问题。
7. defaultServlet：如果请求资源都不匹配对象名，则按照文件路径找资源。

ps：在SpringMVC中如果匹配不到对象名，也不会按照静态方式找资源，需要自己处理才可以。
...
+ abstract class HttpServlet extends GenericServlet  HttpServlet抽象类,除了基本的实现以外,增加了更多的基础功能
    ...
    + public void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
        ...
        + 在该方法中,通过请求方式（比如POST，GET，PUT，DELETE等）判断,调用具体的do***方法完成请求的处理
...
同样能实现页面跳转，优先使用响应重定向。一般请求转发用于访问WEB-INF的资源。
...
+ 相对路径
    + 相对路径的规则是: 以==当前资源所在的路径==为出发点去寻找目标资源
    + 相对路径不以 / 开头
    + 在file协议下,使用的是磁盘路径
    + 在http协议下,使用的是url路径
