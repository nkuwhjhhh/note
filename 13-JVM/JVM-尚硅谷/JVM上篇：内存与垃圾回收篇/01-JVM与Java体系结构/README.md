...
首先，因为基于栈式架构的设计与实现相对比较简单，其次在资源受限的平台可以使用，那么对于不局限与嵌入式的平台也可以使用，没有必要去更换架构。

基于栈的架构跨平台性好、指令集小，虽然相对于基于寄存器的架构来说，基于栈的架构编译得到的指令更多，执行性能也不如基于寄存器的架构好，但**考虑到其跨平台性与移植性**，我们还是选用栈的架构。
